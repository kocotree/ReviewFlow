...
- **AI provider abstraction**: All four providers use a `score()` method returning `{score, detail, dimensions}`; the provider is selected by `AI_PROVIDER` env var. OpenAI-compatible providers (OpenAI, DeepSeek, Doubao) use the `openai` SDK; Claude uses the `anthropic` SDK.
